...
int heater1On = 10; // Heater 1 on when below this temperature
int heater1Off = 11; // Heater 1 off when above this temperature
int heater2On = 9; // Heater 2 on when below this temperature
...
char timeServer[] = "192.168.1.1"; // was 1.12
...
void setup() {
 // Open serial communications and wait for port to open:
  ...
  Serial.println( "Greenhouse mgr v" VERSION_STR );
      ...
      if (client.available()) 
      {
        ...
        if (c == '\n' && currentLineIsBlank) {
          ...
          ****/
          ...
          client.println( "<h5>GHmgr v" VERSION_STR " " + hms(time_utc_sec + utcOffset) + " " + SecondsPlusMS(time_utc_sec, time_utc_ms) + " " + SecondsPlusMS(ntpState/1000, ntpState%1000) + " " + SecondsPlusMS(curMillis/1000, curMillis%1000) + "</h5>" );
          client.println( "<h6>Current</h6>" );
          client.print( "<p>In: ");
          client.print( lastDHT[lastDHTIdx] );
          client.print( " h1{" + String(heater1On) + "," + String(heater1Off) + "} h2{" + String(heater2On)+","+String(heater2Off)+"}");
...
int loop_task()
{
  ...
  long decTime = hms_hour * 10000 + hms_minute * 100 + hms_second;
  timedValve( 2, decTime, 73000, 73100, 9 );
...
int setHeater( int index, int lowTemp, int highTemp )
{
  ...
  if (insideTemp < lowTemp && relaySetting[index] == 0 && time_utc_sec - lastRelayChange[index] >= 10)
  {
    ...
    Serial.println( "H on " + String(index+1) );
  ...
  else if (insideTemp > highTemp && relaySetting[index] == 1 && time_utc_sec - lastRelayChange[index] >= 30)
  {
    ...
    Serial.println( "H off " + String(index+1) );
...
int timedValve( int index, long decTime, long decTimeOn, long decTimeOff, int minTemp )
{
  ...
  if (decTime < decTimeOn || decTime >= decTimeOff)
  {
    ...
    {
      Serial.println("Assert off " + String(index));
      relaySetting[index] = 0;
      digitalWrite( pins[index], HIGH );
      return 1;
    }
   ...
   {
     Serial.println("Assert on " + String(index));
     relaySetting[index] = 1;
     digitalWrite(pins[index], LOW);
     return 1;
   }
